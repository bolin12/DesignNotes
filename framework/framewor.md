...
相比于从服务器端训练模型和验证，嵌入式设备部署会遇到很多问题，首先是由于算力不同导致的帧率的巨大差异，因此需要做模型的剪枝和压缩操作。在精度范围允许的条件下进行减少参数计算量的措施。

模型压缩常见操作有:分组卷积，


去掉部分模型分支，以及对卷积网络进行提前降维，采用1X1的卷积操作，分离卷积等等。
...
本设计采用嵌入式设备，优化调整目标分割网络的网络超参数，提升检测准确率和召回率。

通过模型剪枝操作，优化网络的结构和参数量，在模型精度损失允许的范围内，提升检测速率。

配置并部署嵌入式设备的环境，在PC上完成模型的训练后部署到嵌入式设备上。
...
本设计才用了基于图像分割的神经网络，网络结构采用Unet设计，并加上不同尺度的特征图之间的连接，整体采用堆叠网格的结构，
